...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Color _getStatusColor(String? status) {
    if (status == null || status.isEmpty || status.toLowerCase() == 'none') {
      // Send Request - Primary color
  ...
  Color _getStatusBorderColor(String? status) {
    if (status == null || status.isEmpty || status.toLowerCase() == 'none') {
      // Send Request - Primary color with opacity
      return AppColors.primaryColor.withOpacity(0.5);
    }

    switch (status.toLowerCase()) {
      case 'accepted':
        // Connected - Success green with opacity
        return Colors.green.withOpacity(0.5);
      case 'pending':
        // Request Sent - Warning orange with opacity
        return Colors.orange.withOpacity(0.5);
      case 'rejected':
        // Rejected - Error red with opacity
        return Colors.red.withOpacity(0.5);
      default:
        // Default - Grey with opacity
  ...
  Color _getStatusBackgroundColor(String? status) {
    if (status == null || status.isEmpty || status.toLowerCase() == 'none') {
      // Send Request - Primary color with low opacity
      return AppColors.primaryColor.withOpacity(0.1);
    }

    switch (status.toLowerCase()) {
      case 'accepted':
        // Connected - Success green with low opacity
        return Colors.green.withOpacity(0.1);
      case 'pending':
        // Request Sent - Warning orange with low opacity
        return Colors.orange.withOpacity(0.1);
      case 'rejected':
        // Rejected - Error red with low opacity
        return Colors.red.withOpacity(0.1);
      default:
        // Default - Grey with low opacity
              ...
              Expanded(
                child: Obx(() {
                  if (controller.selectedTab.value == 0) {
                            ...
                            children: [
                              ...
                              DecoratedBox(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    _getStatusText(user.friendRequestStatus),
                                    style: TextStyle(
                    ...
                    final list =
                        controller.searchComunityModel.value.data ?? [];
                    return ListView.separated(
                      itemCount: list.length,
                      padding: const EdgeInsets.fromLTRB(0, 20, 0, 0),
                      separatorBuilder: (context, idx) => SizedBox(height: 16),
                            ...
                            children: [
                              ...
                              DecoratedBox(
                                ...
                                AppDialogs.showConfirmationDialog(
                                  ...
                                  description:
                                      AppStrings.dialogJoinCommunityDescription,
