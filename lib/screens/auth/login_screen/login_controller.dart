...
class LoginController extends GetxController {
  ...
  Future<void> login(GlobalKey<FormState> formKey) async {
    ...
    try {
      ...
      if (loginResponse != null &&
        ...
        // ✅ Store user data
        print('Token set in headers: Bearer ${loginResponse.data?.token ?? ''}');
        StorageHelper().setAuthToken = loginResponse.data?.token ?? '';
        StorageHelper().isLoggedIn = true;
        StorageHelper().setUserId = loginResponse.data?.userId ?? -1;
  ...
  Future<void> _handleGoogleSignIn(GoogleSignInAccount googleUser) async {
    ...
    try {
      final userCredential = await auth.signInWithCredential(credential);
      if (userCredential.user != null) {
        SocailLoginResponse? loginResponse = await _socialLogin(
          AppStrings.google,
          googleUser.email,
        ...
        if (loginResponse != null &&
          ...
          StorageHelper().isLoggedIn = true;
          StorageHelper().setUserId = loginResponse.userId ?? -1;
          StorageHelper().setUserEmail = googleUser.email;
  ...
  Future<void> _signInWithAppleCredential(
    ...
    try {
      ...
      SocailLoginResponse? loginResponse =
      await _socialLogin(AppStrings.apple, email, fullName);
  ...
  // login user for social login
  Future<SocailLoginResponse?> _socialLogin(
      String loginType, String email, String fullname) async {
    try {
      // ✅ Get FCM token from storage
      final fcmToken = StorageHelper().getFcmToken;
      ...
      SocailLoginResponse loginResponse =
      SocailLoginResponse.fromJson(response);
      return loginResponse;
    } catch (e) {
      debugPrint('Error in social loginUser: $e');
      return null;
  ...
  /// Triggers external validation for each field
  void _triggerExternalValidation() {
    _updateExternalValidationForField(
        AppStrings.emailAddress, emailController.text, emailValidator);
    _updateExternalValidationForField(
