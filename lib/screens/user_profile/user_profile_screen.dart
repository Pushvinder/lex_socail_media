                  ...
                  SliverPadding(
                    ...
                    sliver: SliverGrid(
                      ...
                      delegate: SliverChildBuilderDelegate(
                        childCount: isPhotosTab ? _userProfileController.imagePostList.value.length : _userProfileController.videoPostList.value.length,
                          (BuildContext context, int index) {

                            // photos post tab grid view
                        if (isPhotosTab) {
                          if (index >=  _userProfileController.imagePostList.value.length
                              ) return const SizedBox.shrink();
                          final imageUrl = 
                       _userProfileController.imagePostList[index].imagepath?[0];
                          return GestureDetector(
                            onTap: () {
                              Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (_) => FullScreenImageViewer(
                                    images: _userProfileController.imagePostList[index].imagepath ?? [],
                                    initialIndex: 0,
                                  ),
                                ),
                              );
                          ...
                          // Videos
                          if (index >=  _userProfileController.videoPostList.length
                              ||
                              index >= _userProfileController. videoControllers.length ||
                              index >= _userProfileController.isVideoInitialized.length ||
                              index >= _userProfileController.isVideoError.length) {
                            return _buildGridErrorPlaceholder(
                                "Invalid video index");
                          }
                          final controller = _userProfileController. videoControllers[index];
                          final isInitialized = _userProfileController. isVideoInitialized[index];
                          final hadError = _userProfileController. isVideoError[index];
