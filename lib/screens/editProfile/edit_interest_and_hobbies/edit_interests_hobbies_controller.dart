...
import '../../profile_creation/community_preferences/community_preferences_screen.dart';
import '../edit_community_preferences/edit_community_preferences_screen.dart';

class EditInterestsHobbiesController extends GetxController {
  ...
  void onInit() {
    _getProfileDetails();
    Future.wait([_getInterest(), _getHobbies()]);
    super.onInit();
  }
    ...
    if(user.value.data != null){
      if(interestsList.isNotEmpty){
        user.value.data?.interests?.forEach((element){
          var interestItem =  interestsList.firstWhere((data)=> data.name?.trim() == element.name?.trim());
      ...
      if(hobbiesList.isNotEmpty){
        user.value.data?.hobbies?.forEach((element){
          var hobbiesItem =  hobbiesList.firstWhere((data)=> data.name?.trim() == element.name?.trim());
          if(hobbiesItem != null){
            selectedHobbies.add(hobbiesItem);
            selectedHobbies.refresh();
          }
        });
      }
    }
  ...
  Future<void> onNext() async {
    ...
    if (!(response.any((e) => e == null)) &&
      ...
      Get.to(() => const EditCommunityPreferencesScreen());
    } else {
      Get.snackbar(
        AppStrings.error,
        response
