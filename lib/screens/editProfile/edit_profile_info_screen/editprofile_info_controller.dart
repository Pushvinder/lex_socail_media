...
import 'package:the_friendz_zone/widgets/extentions/date_extension.dart';
import 'package:the_friendz_zone/widgets/extentions/string_extension.dart';

import '../../../config/app_config.dart';
import '../../profile/models/user_response.dart';
import '../../profile_creation/invest_and_hobbies/interests_hobbies_screen.dart';
import '../edit_interest_and_hobbies/edit_interests_hobbies_screen.dart';
...
class EditProfileInfoController extends GetxController {
  ...
  void setUserData(){
    if(user.value.data != null){
      usernameController.text = user.value.data?.username ?? "";
      dateOfBirthController.text = user.value.data?.dob?.toDateTime().toLongDateFormat() ?? "";
      bioController.text = user.value.data?.bio ?? "";
    }
  }
  ...
  Future<void> continueToNext() async {
    try {
      ...
      if (response != null && response.status == AppStrings.apiSuccess) {
        ...
        Get.to(() => EditInterestsHobbiesScreen());
      } else {
  ...
  Future<VerifyOtpResponse?> _registerUser() async {
    try {
      ...
      var result = await ApiManager.callPostWithFormData(
          ...
          fileKey: profileImage.value != null ? ApiParam.profile : null,
          filePaths:  profileImage.value != null ? [profileImage.value!.path] : null);
